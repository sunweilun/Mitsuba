...
Float Path::miWeightBaseNoSweep_GBDPT(const Scene *scene, const Path &emitterSubpath,
	...
	int s, int t, bool sampleDirect, bool lightImage, Float jDet, Float exponent, double geomTermX, double geomTermY, int maxT, float th) {
...
Float Path::miWeightGradNoSweep_GBDPT(const Scene *scene, const Path &emitterSubpath,
	...
	if (s > 0) {
		...
		pdfRad[pos] = vs->evalPdf(scene, vt, vsPred, ERadiance, vsMeasure)* emitterSubpath.edge(s - 1)->pdf[ERadiance];
		offsetPdfRad[pos++] = o_vs->evalPdf(scene, o_vt, o_vsPred, ERadiance, vsMeasure)* offsetEmitterSubpath.edge(s - 1)->pdf[ERadiance];
	}
...
Float Path::miConnectWeight(const Scene *scene, const Path &emitterSubpath,
		const PathEdge *connectionEdge, const Path &sensorSubpath,
		int s, int t, bool sampleDirect, bool lightImage, float radius) {
	int k = s+t+1, n = k+1;
...
Float Path::miMergeWeight(const Scene *scene, const Path &emitterSubpath,
		const PathEdge *connectionEdge, const Path &sensorSubpath,
		int s, int t, bool sampleDirect, bool lightImage, float radius) {
	int k = s+t+1, n = k+1;
