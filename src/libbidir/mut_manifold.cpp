...
bool ManifoldPerturbation::manifoldWalk(const Path &source, Path &proposal, int step, int b, int c,
        bool updateAll, Float rs, Float rp) {
    ...
    p0 = m_manifold->getPosition(1);
    if (!m_manifold->moveInRange(vb_new->getPosition(), n, rp)) {
        ++statsMWFailMove;
        return false;
    }
    if (updateAll) {
    ...
    if (relerr > 10.0f * MTS_MANIFOLD_EPSILON && (p0 - p1).length() > 2 * rs) {
