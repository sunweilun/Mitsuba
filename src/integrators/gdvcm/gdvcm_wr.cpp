...
GDVCMWorkResult::GDVCMWorkResult(const GDVCMConfiguration &conf,
    ...
    if (m_hasLightImage) {
        ...
        for (size_t i = 0; i < m_lightImage.size(); ++i) {
...
GDVCMWorkResult::~GDVCMWorkResult() {
}

void GDVCMWorkResult::put(const GDVCMWorkResult *workResult) {
    for (size_t i = 0; i < m_block.size(); ++i) {
        m_block[i]->put(workResult->m_block[i].get());
        if (m_hasLightImage && i < m_lightImage.size())
            m_lightImage[i]->put(workResult->m_lightImage[i].get());
    }
}

void GDVCMWorkResult::clear() {
    for (size_t i = 0; i < m_block.size(); ++i) {
        if (m_hasLightImage && i < m_lightImage.size())
...
void GDVCMWorkResult::load(Stream *stream) {
    for (size_t i = 0; i < m_block.size(); ++i) {
        if (m_hasLightImage && i < m_lightImage.size())
...
void GDVCMWorkResult::save(Stream *stream) const {
    for (size_t i = 0; i < m_block.size(); ++i) {
        if (m_hasLightImage && i < m_lightImage.size())
...
std::string GDVCMWorkResult::toString() const {
    return m_block[0]->toString(); //todo
}

MTS_IMPLEMENT_CLASS(GDVCMWorkResult, false, WorkResult)
MTS_NAMESPACE_END
