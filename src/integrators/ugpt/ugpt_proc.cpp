...
class UGPTBlockRenderer : public WorkProcessor {
public:

    UGPTBlockRenderer(Bitmap::EPixelFormat pixelFormat, int channelCount, int blockSize,
    ...
    UGPTBlockRenderer(Stream *stream, InstanceManager *manager) {
    ...
    void prepare() {
        ...
        m_integrator = static_cast<UnstructuredGradientPathIntegrator *> (getResource("integrator"));
    ...
    void process(const WorkUnit *workUnit, WorkResult *workResult,
        ...
        UGPTWorkResult *block = static_cast<UGPTWorkResult *> (workResult);
    ...
    ref<WorkProcessor> clone() const {
        return new UGPTBlockRenderer(m_pixelFormat, m_channelCount,
...
private:
    ...
    ref<UnstructuredGradientPathIntegrator> m_integrator;
...
UGPTRenderProcess::UGPTRenderProcess(const RenderJob *parent, RenderQueue *queue, int blockSize, const UnstructuredGradientPathTracerConfig &config)
: BlockedRenderProcess(parent, queue, blockSize),
m_config(config) {
}

ref<WorkProcessor> UGPTRenderProcess::createWorkProcessor() const {
    return new UGPTBlockRenderer(m_pixelFormat, m_channelCount, m_blockSize, m_borderSize, m_warnInvalid);
}

void UGPTRenderProcess::processResult(const WorkResult *result, bool cancelled) {
    const UGPTWorkResult* block = static_cast<const UGPTWorkResult *> (result);
...
MTS_IMPLEMENT_CLASS(UGPTRenderProcess, false, BlockedRenderProcess)
MTS_IMPLEMENT_CLASS_S(UGPTBlockRenderer, false, WorkProcessor)
MTS_NAMESPACE_END
